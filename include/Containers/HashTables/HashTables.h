#pragma once
#include "Containers/HashTables/HashTableOpenAddress.h"

namespace Containers {
    template<typename HashTable>
    concept HashTableType = std::destructible<HashTable> && requires(
    ...
    template<typename Hash, typename ValueType>
    concept HashType = requires(Hash& hash, const ValueType& val) {
        {hash.operator()(val)} -> std::convertible_to<size_t>;
    };
}