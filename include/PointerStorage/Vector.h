...
namespace Containers {
	...
	template <typename T, TypedAllocatorConcept<T> Alloc = TypedAllocator<T>,
		LifetimeManagerConcept<T> Life = LifetimeManager<T>, size_t s_initialCapacity = 16, float s_growthFactor = 1.618f>
	...
	public:
		...
		Vector(InitializerList<T> list) {
			...
			this->rangeÑonstruct(this->size(), list.begin());
		}
		...
		Vector(const Vector& other) {
			...
			this->rangeÑonstruct(this->size(), other.data());
		}
		...
		Vector& operator=(const Vector& other) requires std::is_copy_constructible_v<T> {
			...
			{
				this->deallocate();
				this->allocate(this->size() * s_growthFactor);
			}
			this->rangeÑonstruct(this->size(), other.data());
			return *this;
		}
		...
		void reserve(SizeType capacity) requires
